...
def apply_party(player_index: int, party: list[dict]) -> bool:
    ...
    Regeln:
    - Manuelle Spitznamen werden via PID-Match beibehalten.
    ...
    with _state_lock:
        ...
        old_team = p.get("team", [])
        old_by_pid = {m.get("pid"): m for m in old_team if m.get("pid")}
...
function addMon(pi, list, p) {
  state.players[pi][list].push({id: p.id, de: p.de, nick: p.de, lvl: 5});
  save(); render();
}
...
function renderList(pi, list, arr) {
  ...
  arr.forEach((m, mi) => {
    const div = document.createElement('div');
    div.className = 'mon' + (list==='tot' ? ' dead' : '');
    div.innerHTML = `
      <img src="${SPRITE(m.id)}">
      <div class="info">
        <input class="nick" value="${escapeHtml(m.nick)}" oninput="state.players[${pi}].${list}[${mi}].nick=this.value; save()">
        <div style="display:flex; gap:6px; align-items:center;">
          <span style="color:#888; font-size:0.8em;">${escapeHtml(m.de)}</span>
          <span style="color:#888; font-size:0.8em;">Lv</span>
          <input class="lvl" type="number" value="${m.lvl}" oninput="state.players[${pi}].${list}[${mi}].lvl=parseInt(this.value)||0; save()">
        </div>
      </div>
      <div class="actions">
